...
# Initialize W&B
wandb.init(project="ssl-training-project")

SSLDefaultModelConfig = {
    'objective': 'SSL',
    'mask_node_rate': 0.75,
...
def inference(model, dataloader, split, device, batch_size, order_required=False):
    ...
    with torch.no_grad():
        ...
        for i, data_dict in enumerate(dataloader):
            ...
            print('idx in ssl.py:', idx)
            print('len(idx) in ssl.py:', len(idx))
            print('batch_size in ssl.py:', batch_size)
...
class SSLPipeline(Pipeline):
    def __init__(self,
                 pretrain_prefix: str,
                 overwrite_config: dict = SSLDefaultModelConfig,
    ...
    def fit(self, adata: ad.AnnData,
        ...
        config = SSLDefaultPipelineConfig.copy()
        if train_config:
        ...
        for epoch in tqdm(range(config['epochs'])):
            ...
            for i, data_dict in enumerate(dataloader):
                ...
                x_dict = XDict(input_dict)
                print('x_dict[x_seq].shape root:', x_dict['x_seq'].shape)
                # print('len(data_dict[gene_list]):', len(data_dict['gene_list']))
                # print('len(data_dict[gene_mask]):', len(data_dict['gene_mask']))
                # print('len(x_dict[gene_mask]):', len(x_dict['gene_mask']))
                # print('data_dict[gene_list]:', data_dict['gene_list'])
                # print('before entering the model x_dict:', x_dict)
                # print('before entering the model data_dict:', data_dict)
                print('x_dict.keys() first entry:', x_dict.keys())
            ...
            wandb.log({"epoch": epoch, "train_loss": train_loss[-1]})  # Log train loss to W&B
            ...
            wandb.log({"epoch": epoch, "valid_loss": valid_loss[-1]})  # Log valid loss to W&B
        ...
        self.fitted = True
        print('self in imputation11111111111111111111111111111111111111111111111111111111111111111111111111:', self)
