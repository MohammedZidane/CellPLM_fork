...
class ImputationPipeline(Pipeline):
    ...
    def fit(self, adata: ad.AnnData,
        ...
        dataset = TranscriptomicDataset(adata, split_field, covariate_fields, label_fields, batch_protein_list)
        ...
        for epoch in tqdm(range(config['epochs'])):
            ...
            for i, data_dict in enumerate(dataloader):
                ...
                x_dict = XDict(input_dict)
                out_dict, loss = self.model(x_dict, data_dict['protein_list'])
                optim.zero_grad()
                loss.backward()
    ...
    def predict(self, adata: ad.AnnData,
        ...
        if 'target_proteins' in inference_config:
            target_mask = torch.tensor(
                [adata.var.index.get_loc(g) for g in inference_config['target_proteins']]).long().to(pred.device)
    ...
    def score(self, adata: ad.AnnData,
        ...
        pred = inference(self.model, dataloader, None, device,
                         config['max_eval_batch_size'], order_required=True)['pred']
        if 'target_proteins' in evaluation_config:
            target_mask = torch.tensor(
                [adata.var.index.get_loc(g) for g in evaluation_config['target_proteins']]).long().to(pred.device)
            pred = pred[:, target_mask]
        ...
        assert labels.shape[1] == pred.shape[
            1], f'Inconsistent number of proteins between prediction ({pred.shape[1]}) and labels ({labels.shape[1]}). Please check: (1) Correct target protein list is provided in evaluation_config["target_proteins"]. (2) Correct ground-truth protein expressions are provided in .obsm[label_fields[0]].'
