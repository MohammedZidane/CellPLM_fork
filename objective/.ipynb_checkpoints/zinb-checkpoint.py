...
class NBImputationLoss(nn.Module):
    ...
    def forward(self, out_dict, x_dict):
        eps = 1e-10
        if 'protein_mask' not in x_dict:
            x_dict['protein_mask'] = torch.arange(x_dict['x_seq'].shape[1]).to(x_dict['x_seq'].device)
        mean = out_dict['mean']
        disp = out_dict['disp']
        if 'input_protein_mask' in x_dict:
            mean = mean[:, x_dict['input_protein_mask']]
            disp = disp[:, x_dict['input_protein_mask']]
        out_dict['pred'] = mean
        mean = mean[:, x_dict['protein_mask']]
        disp = disp[:, x_dict['protein_mask']]
        truth = x_dict['x_seq'].to_dense()[:, x_dict['protein_mask']]
...
class NBDenoisingLoss(nn.Module):
    ...
    def forward(self, out_dict, x_dict):
        ...
        truth = x_dict['label']
        mean = out_dict['mean'][:, x_dict['protein_mask']]
        disp = out_dict['disp'][:, x_dict['protein_mask']]
