...
def dense_scipy_to_tensor(x: scipy.sparse.csr_matrix):
    # Convert scipy sparse matrix to a dense numpy array, then to a PyTorch tensor
    return torch.tensor(x.toarray()).float()
...
class TranscriptomicDataset(Dataset):
    def __init__(self, adata: ad.AnnData,
        ...
        self.coord_list = []
        self.order_list = []
        self.batch_protein_list = batch_protein_list
        ...
        if self.batch_protein_list:
            ...
            self.batch_protein_mask = {}
            g2id = dict(zip(self.protein_list, list(range(len(self.protein_list)))))
            for batch in batch_protein_list:
                idx = torch.LongTensor([g2id[g] for g in batch_protein_list[batch]])
                self.batch_protein_mask[batch] = torch.zeros(len(g2id)).bool()
                self.batch_protein_mask[batch][idx] = True
        else:
            self.batch_protein_mask = None
        ...
        for batch in range(batch_labels.max() + 1):
            ...
            self.seq_list.append(dense_scipy_to_tensor(x))
    ...
    def __getitem__(self, idx):
        ...
        if self.batch_protein_mask:
            return_dict['protein_mask'] = self.batch_protein_mask[self.batch_list[idx]]

        return_dict['protein_list'] = self.protein_list
...
class SCDataset(Dataset):
    def __init__(self, tensor_dir='/', protein_set=None):
        ...
        if 'protein_list' in self.batch_metadata:
            del self.batch_metadata['protein_list']
        self.tensor_dir = tensor_dir
        self.isddp = False
        self.bid2did = dict(zip(self.batch_metadata['batch_id'], self.batch_metadata['dataset_id']))
        self.did2protein = dict(zip(self.dataset_metadata['id'], self.dataset_metadata['protein_list']))
        if protein_set:
            protein_mask = []
            for i in self.dataset_metadata['protein_list']:
                i = set(i)
                protein_mask.append(torch.tensor([j in i for j in protein_set]).bool())
            self.did2mask = dict(zip(self.dataset_metadata['id'], protein_mask))
        else:
            self.did2mask = None
    ...
    def __getitem__(self, idx):
        ...
        protein_mask = self.get_protein_mask(self.batch_metadata['dataset_id'][idx]) if self.did2mask else torch.ones(
            [seq.shape[1]]).bool()
        return seq, coord, batch_id, dataset_id, protein_mask

    def get_protein_list(self, dataset_id):
        return self.did2protein[dataset_id]

    def get_protein_mask(self, dataset_id):
        assert self.did2mask, 'protein_set was not passed when created dataset.'
...
class SCPartitionDataset(Dataset):
    def __init__(self, batch_metadata, tensor_dir, idx, protein_set=None):
        self.batch_metadata = {}
...
def stratified_sample_proteins_by_sparsity(data, boundaries=None, seed=10):
    df = data.to_df()
    ...
    protein_group = pd.cut(zero_rates, boundaries, labels=False)
    # protein_df = pd.DataFrame({'zero_rates': zero_rates, 'protein_group': protein_group})
    zero_rates = zero_rates.groupby(protein_group, group_keys=False)
    samples = zero_rates.apply(lambda x: x.sample(min(len(x), 25), random_state=seed))
    return list(samples.index)


def data_setup(adata, return_sparse=False, device='cpu'): # set return_sparse = False as we are working with proteomics not transcriptomics
